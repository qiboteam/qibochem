...
def _sort_excitations(excitations: list[[list[int]]]) -> list[[list[int]]]:
    ...
    if order > 2:
        raise_error(
            NotImplementedError, "Can only handle single and double excitations"
        )
    ...
    def sorting_fn(x):
        ...
        return sum(
            (order + 1 - _i) * abs(x[2 * _i + 1] // 2 - x[2 * _i] // 2)
    ...
    while copy_excitations:
        if not prev:
            ...
            pair_excitations = [
                ...
                if sum(
                    abs(_ex[2 * _i + 1] // 2 - _ex[2 * _i] // 2) for _i in range(order)
                )
                == 0
            ]
            ...
            copy_excitations = sorted(
                copy_excitations, key=sorting_fn if order != 1 else None
            )
        else:
            ...
            same_mo_ex = [
                sorted(list(_f) + list(_t))
                for _f in (_from, new_from)
                for _t in (_to, new_to)
            ]
