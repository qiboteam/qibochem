...
def he_circuit(
    ...
    **kwargs,
    ...
    Args:
        ...
        parameter_gates (Iterable[str | Gate] | None): Single-qubit rotation gates used in the ansatz. These can be
            given either as strings representing valid one-qubit gates, or as :class:`qibo.gates.Gate` directly.
            Default: ``["RY", "RZ"]``
        entangling_gate (str | Gate): Two-qubit entangling gate used in the ansatz. This can be given either as strings
            representing valid two-qubit gates, or as a :class:`qibo.gates.Gate` directly. Default: ``"CNOT"``
        architecture (str): Architecture of the entangling layer, with the possible options: ``"diagonal"``,
            ``"even_layer"``, ``"next_nearest"``, ``"odd_layer"``, ``"pyramid"``, ``"shifted"``, ``"v"``, and ``"x"``
            (defined only for an even number of qubits. Default: ``"diagonal"``
        closed_boundary (bool): If ``True`` (default) and ``architecture not in ["pyramid", "v", "x"]``, adds a
            closed-boundary condition to the entangling layer
        kwargs (dict): Additional arguments used to initialize a Circuit object. Details are given in the
