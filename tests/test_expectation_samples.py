...
def test_measurement_grouping_extra_tests():
    """Additional tests for some previously undiscovered bugs"""
    for method in ("gc", "gc2"):
        hamiltonian = SymbolicHamiltonian(0.5 * X(0) * Y(1) + Z(0) * Z(1) * Z(2), nqubits=3)
        circuit = Circuit(3)
